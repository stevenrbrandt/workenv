...
def strip_grok_installer_path_block(path):
    """Remove the official grok installer PATH block; workenv PATH covers PREFIX/bin."""
    if not os.path.exists(path):
        return
    with open(path, "r") as fd:
        text = fd.read()
    if ">>> grok installer >>>" not in text:
        return
    new, n = re.subn(
    ...
    if n:
        with open(path, "w") as fd:
            fd.write(new)
        print("Removed grok installer PATH block from", path)


strip_grok_installer_path_block(bashrc)
strip_grok_installer_path_block(os.path.join(home, ".bash_profile"))
...
# Always align with platform when unset, on the default ~/.local tree, or on
# a previous workenv platform path (host vs container libc can differ).
if [ -z "${{PYTHONUSERBASE:-}}" ] || [ "$PYTHONUSERBASE" = "$HOME/.local" ]; then
    ...
    case "$PYTHONUSERBASE" in
      "$HOME/.local/"*-glibc-*|"$HOME/.local/"*-musl|"$HOME/.local/"*-unknown-libc)
        export PYTHONUSERBASE="$HOME/.local/$WORKENV_PLATFORM"
        ;;
    esac
fi
...
# Exact prefix first, then older same-arch glibc prefixes (see workenv_compatible_bin_path)
# so host vim/node still work inside a container whose prefix is not built yet.
if command -v workenv_compatible_bin_path >/dev/null 2>&1; then
    _workenv_bins="$(workenv_compatible_bin_path)"
else
    _workenv_bins="$WORKENV_PREFIX/bin"
fi
export PATH="$_workenv_bins:$WORKENV_ROOT/bin:$HOME/bin:$PATH"
...
# Completions from the official grok installer (binaries live in $WORKENV_PREFIX).
...
def ensure_prefix_cli_bins():
    """Put grok/claude launchers in $WORKENV_PREFIX/bin instead of ~/.local."""
    prefix_bin = os.path.join(workenv_prefix(), "bin")
    os.makedirs(prefix_bin, exist_ok=True)

    grok_src = os.path.join(home, ".grok", "bin", "grok")
    grok_dst = os.path.join(prefix_bin, "grok")
    agent_dst = os.path.join(prefix_bin, "agent")
    if os.path.exists(grok_src):
        target = os.path.realpath(grok_src)
        if os.path.isfile(target):
            for dst in (grok_dst, agent_dst):
                if os.path.lexists(dst):
                    if os.path.islink(dst) and os.path.realpath(dst) == target:
                        continue
                    os.remove(dst)
                os.symlink(target, dst)
                print("Linked", dst, "->", target)
            for shim in (
    ...
    claude_src = os.path.join(home, ".local", "bin", "claude")
    claude_dst = os.path.join(prefix_bin, "claude")
    if os.path.lexists(claude_src):
        real = os.path.realpath(claude_src)
        if os.path.isfile(real) and os.access(real, os.X_OK):
            ver = os.path.basename(real)
            ver_dir = os.path.join(workenv_prefix(), "share", "claude", "versions")
            os.makedirs(ver_dir, exist_ok=True)
            ver_path = os.path.join(ver_dir, ver)
            if not os.path.isfile(ver_path):
                shutil.copy2(real, ver_path)
                os.chmod(ver_path, 0o755)
            if os.path.lexists(claude_dst):
                os.remove(claude_dst)
            os.symlink(ver_path, claude_dst)
            print("Linked", claude_dst, "->", ver_path)
            os.remove(claude_src)
            print("Removed", claude_src)
