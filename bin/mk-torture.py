...
for g in re.finditer(r'ctermfg=(\d+)\s+guifg=(#[a-f0-9]+)  "rgb=(\d+),(\d+),(\d+)', colortab):
    cterm=int(g.group(1))
    gui=g.group(2)
    rgb=(int(g.group(3)), int(g.group(4)), int(g.group(5)))
    ...
    if brightness < 20:
        bglist.append(info)
    elif brightness > 300 and brightness < 500:
        fglist.append(info)

home = Path.home()

colors_txt = f"{home}/colors.txt"

if not os.path.exists(colors_txt):
    print("Run :so $VIMRUNTIME/syntax/hitest.vim")
    exit(1)

symbols = set()
with open(colors_txt, "r") as fd:
    for line in fd.readlines():
        if g := re.match(r'^(\w+)\s*($|->)', line):
            symbols.add(g.group(1))

vim_dir = "/usr/share/vim/vim91/colors"
if os.path.exists(vim_dir):
    for fn in os.listdir(vim_dir):
        if not fn.endswith(".vim"):
            continue
        vim_file = f"{vim_dir}/{fn}"
        with open(vim_file, "r") as fd:
            for line in fd.readlines():
                if g:= re.match(r'^hi\s+(\w+)', line):
                    sym = g.group(1)
                    symbols.add(sym)
...
scheme = f"{home}/.vim/colors/rand.vim"
with open(scheme, "w") as fd:
    fd.write(header)
    for symbol in sorted(symbols):
        fg = fglist[randint(0,len(fglist)-1)]
        bg = bglist[randint(0,len(bglist)-1)]
        fg_cterm = fg["cterm"]
        fg_gui = fg["gui"]
        bg_cterm = bg["cterm"]
        bg_gui = bg["gui"]
...
with open(scheme, "r") as fd:
    contents = fd.read()

from randpass import randpass
while True:
    rand_name = randpass('MN')
    fname = f"{home}/.vim/colors/{rand_name}.vim"
    if os.path.exists(fname):
        continue
    print(rand_name)
    #with open(fname, "w") as fd:
    #    fd.write(contents)
    os.link(scheme, fname)
    break
