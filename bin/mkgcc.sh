...
export LZLIB_VER=1.21
export MPFR_VER=4.0.2
export MPC_VER=1.1.0
export GCC_VER=6.5.0
export PARALLEL=4

set -x
export GCC_BUILD_DIR=/project/sbrandt/chemora
if [ ! -d $GCC_BUILD_DIR ]
then
...
cd $GCC_BUILD_DIR
cd lzip-${LZLIB_VER}
export LZIP=$GCC_BUILD_DIR/install/lzip-${LZLIB_VER}/bin/lzip
if [ ! -x $LZIP ]
then
  ./configure --prefix=$GCC_BUILD_DIR/install/lzip-${LZLIB_VER}
...
cd $GCC_BUILD_DIR
GMP_DIR=${GCC_BUILD_DIR}/install/gmp-${GMP_VER}
...
export MPFR_DIR=${GCC_BUILD_DIR}/install/mpfr-${MPFR_VER}
...
cd $GCC_BUILD_DIR
export MPC_DIR=${GCC_BUILD_DIR}/install/mpc-${MPC_VER}
...
export NVPTX_DIR=${GCC_BUILD_DIR}/install/nvptx
...
cd $GCC_BUILD_DIR
export GCC_DIR=${GCC_BUILD_DIR}/install/gcc-${GCC_VER}
