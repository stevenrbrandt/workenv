...
if [ ! -d "$SPACK_ROOT" ]
then
  git clone --single-branch --branch v0.17.1 --depth 1 https://github.com/spack/spack.git "$SPACK_ROOT"
fi
...
if ! spack find zlib%gcc@${GCC}
then
  spack install zlib%gcc@${GCC}
fi
if [ ! -d "$ENV_DIR" ]
then
...
set +e
spack find gcc@11.2.0%gcc@${GCC}
if [ $? = 0 ]
then
  echo gcc@11.2.0 was found
  set -e
else
  echo gcc@11.2.0 was NOT found
  set -e
...
set +e
rm -fr carpetx
spack view symlink -i carpetx mpich hdf5 libjpeg openssl fftw papi gsl hwloc adios2 amrex boost cuda googletest gperftools kadath likwid memkind nsimd openblas openpmd-api petsc reprimand rnpletal simulationio ssht yaml-cpp gdb pgplot gcc zlib/$ZLIB
